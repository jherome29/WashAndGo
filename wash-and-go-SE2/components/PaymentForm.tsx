...
import { CreditCard, Upload, User, Phone, Mail, Info, UserCheck, CheckCircle } from 'lucide-react';
...
export default function PaymentForm({
  ...
  onBack, onSubmit, submitting, user, token,
}: PaymentFormProps) {
  ...
  return (
    <div className="animate-fade-in grid grid-cols-1 lg:grid-cols-5 gap-8">
      ...
      <div className="lg:col-span-3">
        <h2 className="font-lovelo font-black text-2xl text-gray-900 mb-6">CUSTOMER & PAYMENT</h2>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-between pt-6 border-t">
            ...
            <button type="submit"
              disabled={!name || !phone || !proofFile || (!user && !email) || uploading || submitting}
              ...
              style={name && phone && proofFile && (user || email) && !uploading && !submitting
                ? { background: 'linear-gradient(135deg, #ee4923 0%, #F4921F 100%)' }
                : { backgroundColor: '#d1d5db' }}>
              {uploading ? `Uploading ${uploadProgress}%...` : submitting ? 'Submitting...' : 'COMPLETE BOOKING'}
            </button>
          </div>
        </form>
      </div>
